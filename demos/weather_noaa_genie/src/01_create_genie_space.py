...
This schema has 3 dimension tables and 1 fact table.

### Join Keys
- **stations** is the central dimension. The weather fact table joins via \
`station_id` -> `stations.station_id`.
- **countries** joins via `country_code` <- `stations.country_code`.
- **states** joins via `state_code` <- `stations.state_code`. Only applies to \
